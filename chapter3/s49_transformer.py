...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, e_dim, h_dim, n_heads):
        ...
        """
        super().__init__()
        self.atte_layers = nn.ModuleList(
            [OneHeadAttention(e_dim, h_dim) for _ in range(n_heads)]
        )
...
class TransformerEncoder(nn.Module):
    def __init__(self, e_dim, h_dim, n_heads, n_layers, drop_rate=0.1):
        ...
        self.encoder_layers = nn.ModuleList(
            [EncoderLayer(e_dim, h_dim, n_heads, drop_rate) for _ in range(n_layers)]
        )
...
class TransformerDecoder(nn.Module):
    def __init__(self, e_dim, h_dim, n_heads, n_layers, drop_rate=0.1):
        ...
        self.decoder_layers = nn.ModuleList(
            [DecoderLayer(e_dim, h_dim, n_heads, drop_rate) for _ in range(n_layers)]
        )
...
if __name__ == "__main__":
    ...
    encoder = TransformerEncoder(12, 8, 3, 6)
    querys = encoder(input)
    decoder = TransformerDecoder(12, 8, 3, 6)
    pred_seqs = torch.randn(5, 3, 12)
    ouput = decoder(pred_seqs, querys)
    print(ouput.shape)
