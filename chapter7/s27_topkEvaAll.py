...
from chapter3 import (
    s22_FNN_plus as fnnTrain,
    s24_DeepFM as deepfmTrain,
    s25_AFM as afmTrain,
)
...
modelsTrain = {"afm": afmTrain, "fnn": fnnTrain, "deepfm": deepfmTrain}
...
# 进行普通的模型评估
def doModelEva():
    _, test_set, _, _, _ = dataloader4ml100kIndexs.read_data()
...
def doTopKEva(
    ...
    evaMethods=[
        tke.EvaTarget.precision,
        tke.EvaTarget.precision_full,
        tke.EvaTarget.recall,
        tke.EvaTarget.map,
        tke.EvaTarget.mrr,
        tke.EvaTarget.ndcg,
        tke.EvaTarget.user_hr,
        tke.EvaTarget.item_hr,
    ],
    needPrint=True,
    needDraw=True,
):
    """
    :param model_paths: 要评估的模型地址字典,储存形式是 {模型名：模型文件地址}
    :param ks: 要评估的K值列表
    :param evaMethods: 要评估的指标，缺省是全部
    :param needPrint: 是否要打印出指标
    :param needDraw: 是否要画图
    :return all_dict: 包含所有评估指标的字典，储存形式是 {模型名: {指标名：[对应k值的评估分数...]...}...}
    """
    _, test_set, _, _, _ = dataloader4ml100kIndexs.read_data()
    pos_dct, neg_dct, all_items = tke.fromTripleToSetDict(test_set)
    pairs, pos_set, neg_set = tke.fromSetDictToBocastTestPair(
        pos_dct, neg_dct, all_items
    )
    all_dict = {}
    for model in model_paths:
        net = torch.load(model_paths[model])
        ...
        for u, i in tqdm(DataLoader(pairs, batch_size=len(all_items), shuffle=False)):
            out = net(u, i)
            outs = [
                [round(float(r), 4), i] for r, i in zip(out.detach().numpy(), all_items)
            ]
        ...
        return_dict = tke.doTopKEvaAll(
            all_preds, pos_set, neg_set, evaMethods, needPrint
        )
