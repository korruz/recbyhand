...
def DCG(scores):
    return np.sum(np.divide(np.array(scores), np.log2(np.arange(len(scores)) + 2)))
...
def doTopKEva(all_preds_dict, poss, negs):
    return_dict = {"precision": [], "precision_full": [], "recall": []}
...
def doTopKEvaAll(
    ...
    for k in all_preds_dict:
        ...
        for pred, pos, neg in zip(all_preds_dict[k], poss, negs):
            if (
                EvaTarget.precision in evaMethods
                or EvaTarget.precision_full in evaMethods
            ):
                ...
                if EvaTarget.precision_full in evaMethods:
                    all_p_full += p_full
            begin_values += np.array(
                [et.value[1](pred, pos) for et in evaMethods if et not in sp_evas]
            )
            if EvaTarget.item_hr in evaMethods:
                all_preds |= set(pred)
                all_pos |= set(pos)
