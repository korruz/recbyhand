...
class GAT4Rec(torch.nn.Module):
    def __init__(self, n_users, n_entitys, dim):
        ...
        self.W = nn.Linear(
            in_features=dim, out_features=dim // self.multiHeadNumber, bias=False
        )
    ...
    def oneHeadAttention(self, target_embeddings, neighbor_entitys_embeddings):
        ...
        target_embeddings_broadcast = torch.cat(
            [
                ...
                for _ in range(neighbor_entitys_embeddings.shape[1])
            ],
            dim=1,
        )
        # [ batch_size, n_neighbor, w_dim*2 ]
        cat_embeddings = torch.cat(
            [target_embeddings_broadcast, neighbor_entitys_embeddings_w], dim=-1
        )
    ...
    def multiHeadAttentionAggregator(
        self, target_embeddings, neighbor_entitys_embeddings
    ):
        """
        ...
        for i in range(self.multiHeadNumber):  # 循环多头注意力的头数
            embs.append(
                self.oneHeadAttention(target_embeddings, neighbor_entitys_embeddings)
            )
    ...
    def __getEmbeddingByNeibourIndex(self, orginal_indexes, nbIndexs, aggEmbeddings):
        ...
        for v in orginal_indexes:
            embs = aggEmbeddings[
                torch.squeeze(torch.LongTensor(nbIndexs.loc[v].values))
            ]
    ...
    def gnnForward(self, adj_lists):
        ...
        for df in adj_lists:
            if n_hop == 0:
                ...
                entity_embs = self.__getEmbeddingByNeibourIndex(
                    df.values, neighborIndexs, aggEmbeddings
                )
            # [图采样时的某一阶的中心节点数量, dim ]
