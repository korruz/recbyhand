...
class KGCN(nn.Module):
    def __init__(
        ...
        self.linear_layer = nn.Linear(
            ...
            if self.aggregator_method == "concat"
            else self.e_dim,
    ...
    def forward(self, users, items, is_evaluate=False):
        ...
        # 得到v波浪线
        neighbor_vectors = self.__get_neighbor_vectors(
            neighbor_entitys, neighbor_relations, user_embeddings
        )
        # 聚合得到物品向量
        out_item_embeddings = self.aggregator(
            item_embeddings, neighbor_vectors, is_evaluate
        )
    ...
    # 得到v波浪线
    def __get_neighbor_vectors(
        self, neighbor_entitys, neighbor_relations, user_embeddings
    ):
        # [batch_size, n_neighbor, dim]
        user_embeddings = torch.cat(
            [torch.unsqueeze(user_embeddings, 1) for _ in range(self.n_neighbors)],
            dim=1,
        )
        ...
        user_relation_scores_normalized = torch.unsqueeze(
            user_relation_scores_normalized, 2
        )
        # [batch_size, dim ]
        neighbor_vectors = torch.sum(
            user_relation_scores_normalized * neighbor_entitys, dim=1
        )
...
def train(
    epochs=20, batchSize=1024, lr=0.01, dim=128, n_neighbors=10, eva_per_epochs=1
):
    ...
    adj_entity, adj_relation = dataloader4KGNN.construct_adj(
        n_neighbors, kg_indexes, len(entitys)
    )
    net = KGCN(
        max(users) + 1,
        max(entitys) + 1,
    ...
    for e in range(epochs):
        ...
        for u, i, r in tqdm(DataLoader(train_set, batch_size=batchSize, shuffle=True)):
            ...
            all_loss += loss.item()
        print(
            "epoch {},avg_loss={:.4f}".format(
                e, all_loss / (len(train_set) // batchSize)
            )
        )
