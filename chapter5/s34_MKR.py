...
class CrossCompress(nn.Module):
    ...
    def forward(self, v, e):
        ...
        # [batch_size, dim]
        v_output = torch.matmul(c_matrix, self.weight_vv) + torch.matmul(
            c_matrix_transpose, self.weight_ev
        )
        e_output = torch.matmul(c_matrix, self.weight_ve) + torch.matmul(
            c_matrix_transpose, self.weight_ee
        )
        # [batch_size, dim]
...
def train(epochs=20, batchSize=1024, lr=0.01, dim=128, eva_per_epochs=1):
    ...
    for e in range(epochs):
        ...
        for rec_set, kg_set in tqdm(
            zip(
                DataLoader(
                    train_set, batch_size=batchSize, shuffle=True, drop_last=True
                ),
                DataLoader(
                    kgTrainSet, batch_size=batchSize, shuffle=True, drop_last=True
                ),
            )
        ):
            optimizer.zero_grad()
