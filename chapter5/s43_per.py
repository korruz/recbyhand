...
def getSimMatrixOfAllRelations(metapath_al, movie_set):
    ...
    for r in tqdm(metapath_al):
        metapath_simMatrixs[r] = s42_pathSim.getSimMatrixFromAl(
            metapath_al[r], max(movie_set) + 1
        )
...
class PER(nn.Module):
    def __init__(self, kgTriples, user_set, movie_set, recTriples, dim=8):
        ...
        print("计算用户偏好扩散矩阵...")
        sortedUserItemSims, self.metapath_map = self.init_userItemSims(
            user_set, recTriples, metapath_simMatrixs
        )
        print("初始化用户物品在每个元路径下的embedding...")
        self.embeddings = self.init_embedding(
            dim, sortedUserItemSims, self.metapath_map
        )
    ...
    def init_userItemSims(self, user_set, recTriples, metapath_simMatrixs):
        ...
        for metapath in metapath_simMatrixs:
            for u in userItemAl:
                userItemSims[metapath][u] = np.sum(
                    metapath_simMatrixs[metapath][
                        [i for i in userItemAl[u] if userItemAl[u][i] == 1]
                    ],
                    axis=0,
                )
        ...
        metapath_map = {
            k: v for k, v in enumerate(sorted([metapath for metapath in userItemSims]))
        }
    ...
    def init_embedding(self, dim, sortedUserItemSims, metapath_map):
        ...
        for metapath in metapath_map:
            ...
            user_vectors, item_vectors = self.__init_one_pre_emd(
                sortedUserItemSims[metapath_map[metapath]], dim
            )
            # 分别用先验的用户与物品的向量初始化每个元路径下代表表示用户及表示物品的embedding层
            embeddings[metapath]["user"] = nn.Embedding.from_pretrained(
                user_vectors, max_norm=1
            )
            embeddings[metapath]["item"] = nn.Embedding.from_pretrained(
                item_vectors, max_norm=1
            )
        return embeddings
    ...
    def forward(self, u, v):
        ...
        for metapath in self.metapath_map:
            ...
            # [ batch_size, 1 ]
            metapath_pred = torch.sum(
                metapath_embs["user"](u) * metapath_embs["item"](v), dim=1, keepdim=True
            )
...
def train(epochs=10, batchSize=1024, lr=0.1, eva_per_epochs=1):
    ...
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, net.parameters()), lr=lr, weight_decay=1e-3
    )
    ...
    for e in range(epochs):
        ...
        if e % eva_per_epochs == 0:
            ...
            print(
                "train: Precision {:.4f} | Recall {:.4f} | accuracy {:.4f}".format(
                    p, r, acc
                )
            )
            p, r, acc = doEva(net, test_set)
            print(
                "test: Precision {:.4f} | Recall {:.4f} | accuracy {:.4f}".format(
                    p, r, acc
                )
            )
