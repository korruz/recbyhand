...
class KGFM(nn.Module):
    ...
    def FMMessagePassFromKGAT(self, h_embs, r_embs, t_embs):
        ...
        h_broadcast_embs = torch.cat(
            [torch.unsqueeze(h_embs, 1) for _ in range(t_embs.shape[1])], dim=1
        )
    ...
    def FMMessagePassFromKGCN(self, u_embs, r_embs, t_embs):
        ...
        u_broadcast_embs = torch.cat(
            [torch.unsqueeze(u_embs, 1) for _ in range(t_embs.shape[1])], dim=1
        )
    ...
    def aggregate(self, h_embs, Nh_embs, agg_method="Bi-Interaction"):
        ...
        """
        if agg_method == "Bi-Interaction":
            return self.leakyRelu(self.W1(h_embs + Nh_embs)) + self.leakyRelu(
                self.W2(h_embs * Nh_embs)
            )
