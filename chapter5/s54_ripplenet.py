...
class RippleNet(nn.Module):
    def __init__(
        ...
        if (
            item_update_mode == "replace_transform"
            or item_update_mode == "plus_transform"
        ):
    ...
    def forward(
        ...
        rec_loss = self._get_rec_loss(
            item_embs, labels, h_emb_list, r_emb_list, t_emb_list
        )
...
def get_feed_dict(data, ripple_set):
    ...
    for hop in range(n_hop):
        memories_h.append(
            torch.LongTensor([ripple_set[int(user)][hop][0] for user in u])
        )
        memories_r.append(
            torch.LongTensor([ripple_set[int(user)][hop][1] for user in u])
        )
        memories_t.append(
            torch.LongTensor([ripple_set[int(user)][hop][2] for user in u])
        )
...
def train(n_epoch=n_epoch, batch_size=batch_size, eva_per_epochs=1):
    ...
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, net.parameters()), lr
    )
    ...
    for e in range(n_epoch):
        ...
        print(
            "epoch {},avg_loss={:.4f}".format(
                e, all_loss / (len(train_set) // batch_size)
            )
        )
        ...
        if e % eva_per_epochs == 0:
            ...
            print(
                "train: Precision {:.4f} | Recall {:.4f} | AUC {:.4f}".format(p, r, auc)
            )
            # 给测试集测试时重新生成水波集来增加预测难度
            ripple_set = get_ripple_set(kg_indexs, user_history_pos_dict)
            p, r, auc = doEva(net, test_set, ripple_set, batch_size)
            print(
                "test: Precision {:.4f} | Recall {:.4f} | AUC {:.4f}".format(p, r, auc)
            )
